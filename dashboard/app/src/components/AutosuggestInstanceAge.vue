    ...
    export default {
        ...
        methods: {
            ...
            fetchResults() {
                ...
                this.timeout = setTimeout(() => {
                    ...
                    Promise.all([dataPromise]).then(values => {
                        this.suggestions = [];
                        this.selected = null;
                        const data = this.filterResults(values[0].data.data, query, "val");
                        console.log(data);
                        data.length &&
                        this.suggestions.push({name: "Ethnicities", data: data});
                    });
            ...
            getSuggestionValue(suggestion) {
                ...
                // console.log(item);
                this.selectedAgeGroup = item.val;
                this.$emit('age-selected', this.selectedAgeGroup);
                return item.val;
            }
        },
        watch: {
